...
void stergereLocuinta(struct Locuinta** vector, int* dim, int id) {
	char flag = 0;
	for (int i = 0; i < (*dim); i++) {
		if ((*vector)[i].id == id) {
			flag = 1;
...
void dezalocareMtrice(struct Locuinta*** matrice, int* nr_linii, int** nr_coloane) {
...
void citesteMtriceFisier(const char* nume_fisier, struct Locuinta** matrice, int nr_linii, int* nr_coloane) {
	if (nume_fisier != NULL && strlen(nume_fisier) > 0) {
		FILE* f = fopen(nume_fisier, "r");
		if (f != NULL) {
			char buffer[100];
...
void schimbareNumarLocuinta(struct Locuinta** matrice, int nrLinii, int* nrColoane, int numarLocuinta) {
	...
	if (idLocuinta != -1) {
		...
		stergereLocuinta(&(matrice[pozitie]), &(nrColoane[pozitie]), idLocuinta);
	}
}
...
void main() {
	...
	citesteMtriceFisier("locuinte.txt", matrice, nr_linii, nr_coloane);
	afisareMatrice(matrice, nr_linii, nr_coloane);
	int paritate = 11;
	printf("Id-urile cu paritate impara:\n");
	...
	dezalocareMtrice(&matrice, &nr_linii, &nr_coloane);
